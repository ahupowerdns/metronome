...
is consumed but not stored. Note that only path is matched, scheme, host and url parameters are ignored. 
  ...
  public:
    ...
    bool route(Request *req, Response *resp, THandlerFunction& handler); //<! Instance method for performing routing
...
*/
    ...
    static bool Route(Request *req, Response *resp, THandlerFunction& handler) { return router.route(req, resp, handler); }; //<! Performs routing based on req->url.path 
